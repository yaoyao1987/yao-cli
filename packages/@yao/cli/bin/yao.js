...
checkNodeVersion(requiredVersion, '@yao/cli')

const minimist = require('minimist')
const program = require('commander')

program
  .version(`@yao/cli ${require('../package').version}`)
...
program
  ...
  .action((template, name) => {
    if (minimist(process.argv.slice(3))._.length > 2) {
      console.log(chalk.yellow('\n Info: You provided more than two argument. The first one will be used as the app\'s type, the second one will be used as the app\'s name.'))
    }
    ...
    require('envinfo').run(
      {
        ...
        npmPackages: '/**/{typescript,*yao*,@yao/*/}',
        npmGlobalPackages: ['@yao/cli']
      },
      {
        showNotFound: true,
        duplicates: true,
...
program.on('--help', () => {
  console.log()
  console.log(`  Run ${chalk.cyan(`yao <command> --help`)} for detailed usage of given command.`)
  console.log()
})
