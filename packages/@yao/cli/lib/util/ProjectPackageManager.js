...
class PackageManager {
  constructor({ context, forcePackageManager } = {}) {
    ...
    if (this.bin === 'npm') {
      ...
      if (semver.lt(npmVersion, MIN_SUPPORTED_NPM_VERSION)) {
        throw new Error(
          ...
          'It does not support some core functionalities of Yao CLI.\n' +
    ...
    const resolveFrom = projectPkg && projectPkg.yaoPlugins && projectPkg.yaoPlugins.resolveFrom

    // Logically, `resolveFrom` and `context` are distinct fields.
    // But in Yao CLI we only care about plugins.
  ...
  async runCommand(command, args) {
    ...
    // In the use case of Yao CLI, when installing dependencies,
