const { semver } = require('@yao/cli-shared-utils')
...
async function getAndCacheLatestVersion(cached, includePrerelease) {
  let version = await pm.getRemoteVersion('yao-cli-version-marker', 'latest')

  if (includePrerelease) {
    const next = await pm.getRemoteVersion('yao-cli-version-marker', 'next')
