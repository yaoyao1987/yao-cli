...
const xdgConfigPath = file => {
  ...
  if (xdgConfigHome) {
    const rcDir = path.join(xdgConfigHome, 'yao')
...
// we introduced a change storing .yaorc in AppData, but the benefit isn't
...
const migrateWindowsConfigPath = file => {
  ...
  if (appData) {
    const rcDir = path.join(appData, 'yao')
...
exports.getRcPath = file => {
  ...
  return (
    process.env.YAO_CLI_CONFIG_PATH ||
    xdgConfigPath(file) ||
